...
@section('content')
    <section class="content">
        <div class="box">
            ...
            {!! Form::model($gallery_item,['action'=>['GalleryController@update',$gallery_item->id],'method'=>'patch','class'=>'form-horizontal','files'=>true,'enctype'=>'multipart/form-data']) !!}
            <div class="box-body">
                <div class="row">
                    <div class="col-sm-6 col-sm-offset-2">
                                ...
                                <select name="categories[]" class="select2 form-control" multiple>
                                    @if(isset($GalleryCategory) && $GalleryCategory->count()>0)
                                        @foreach($GalleryCategory as $category)
                                            <?php $gallery_categories = explode(',',$gallery_item['categories']); ?>
                                            <option {{ (in_array( $category['id'],$gallery_categories)?'selected="selected"':'') }} value="{{ $category['id'] }}"> {{ $category['value'] }}</option>
                                        @endforeach
                                    @endif
                        ...
                        <div class="form-group {{ $errors->has('image') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-9">
                                <input class="form-control input-photo" name="image" id="image" placeholder="Image" type="file">
                                @if ($errors->has('image'))
                                    <span class="help-block">
                                    <strong>{{ $errors->first('image') }}</strong>
                                </span>
                            ...
                            <div class="col-sm-9 col-sm-offset-3">
                                <img class="img-thumbnail" id="image_upload_preview" src="{{ asset('assets/images/gallery/'.$gallery_item->image) }}" alt="{{ $gallery_item->title }}">
                            </div>
                        </div>
